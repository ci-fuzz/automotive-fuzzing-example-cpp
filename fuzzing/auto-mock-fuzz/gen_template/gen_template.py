...
def parse_function(function):
    ...
    # Check if this is a constructor or deconstructor
    match = re.match(r'^(.*?)\(.*$', function)
    if " " not in match.group(1):
        print(f"(De)Constructor detected: {function}")
        return {}
    # Parse return type, function name and parameter string
    match = re.match(r'^(.*[\* ])([:\w]+)\((.*)\)(\s:.+)?$', function)
    if not match:
        match = re.match(r'^([~:\w]+)\((.*)\)(\s:.+)?$', function)
        if not match:
...
def main():
    ...
    func_defs = [func_def for func_def in func_defs if func_def != {}]
