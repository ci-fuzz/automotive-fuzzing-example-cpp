...
class FuzzDataGenerator:
    ...
    def gen_string(self):
        var_name = self.gen_var_name('fuzz_string_')
        self.gen_code.append(f'std::string {var_name} = fuzz_data->ConsumeRandomLengthString();')
        return var_name
...
def generate_fuzztest_invocation(func):
    ...
    for p_name, p_type in func['params'].items():
        ...
        elif len(cmd) == 1 and cmd[0] == 'ARG_STRING':
            call_param_values.append(data_gen.gen_string())
